...
class SwaggerV2Generator(SwaggerGenerator):
    ...
    def generate(
        ...
        registry_extensions = getattr(registry, "swagger_extensions", None)
        if registry_extensions:
            deep_merge(swagger, registry_extensions)
    ...
    def _get_paths(
        ...
        for path, methods in paths.items():
            ...
            for method, d in methods.items():
                ...
                func_extensions = getattr(d.func, "__openapi_extensions__", None)
                if func_extensions:
                    deep_merge(path_definition[method_lower], func_extensions)
